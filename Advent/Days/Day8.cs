namespace Advent.Days
{
    internal static class Day8
    {
        public static void RunDay8()
        {
            ...
            {
                currentDisplays.SevenSegmentDisplays.Add(new SevenSegmentDisplay());

                foreach (var signalPattern in sequence.SignalPattern)
                {
                ...
                foreach (var pattern in sequence.OutputPattern)
                {
                    var sequenceMatcher = currentDisplays.SevenSegmentDisplays.SelectMany(x => x.SevenSegmentOptions.Where(y => y.NumberOfAlternativeSegments == pattern.Length).ToList()).ToList();

                    if (sequenceMatcher != null && sequenceMatcher.Count == 1)
                    {
                        totalCountOf1478++;
                    }
                }
            ...
            Console.WriteLine($"Total times 1, 4, 7, 8 appear in sequence = {totalCountOf1478}");
        }
        ...
        private class TestInput
        {
            ...
            {
                var sequences = input.Split(Environment.NewLine);

                foreach (var sequence in sequences)
                {
                    SignalInputs.Add(new SignalInput
                    {
                        SignalPattern = sequence.Split("|").First().Trim().Split(" ").ToList(),
                        OutputPattern = sequence.Split("|").Last().Trim().Split(" ").ToList()
                    });
                }
            }

            public List<SignalInput> SignalInputs { get; set; } = new();
        }

        private class SignalInput
        {
            public List<string> SignalPattern { get; set; } = new();
            public List<string> OutputPattern { get; set; } = new();
        }

        private class CurrentDisplay
        {
            public List<SevenSegmentDisplay> SevenSegmentDisplays { get; set; } = new();
        }
        ...
        private class SevenSegmentDisplay
        {
            ...
            {
                SevenSegmentOptions = new()
                {
                    new SevenSegmentOption
                    {
                        NumberDisplayed = 0,
                        DefaultSegments = new()
                        {
            ...
            public List<SevenSegmentOption> SevenSegmentOptions { get; set; }
        }

        private class SevenSegmentOption
        {
            public int NumberDisplayed { get; set; }
            public int NumberOfSegments => DefaultSegments.Count;
            public int NumberOfAlternativeSegments => AlternativeSegments.Count;
            public List<string> DefaultSegments { get; set; } = new();
            public List<string> AlternativeSegments { get; set; } = new();
        }

        private static readonly string testInput = @"be cfbegad cbdgef fgaecd cgeb fdcge agebfd fecdb fabcd edb | fdgacbe cefdb cefbgd gcbe
...
gcafb gcf dcaebfg ecagb gf abcdeg gaef cafbge fdbac fegbdc | fgae cfgab fg bagce";
        ...
        private static readonly string puzzleInput = @"fdeba beagfd gbafe dagb dbf ecfad bd dgcaefb fbecgd abfecg | dgba dfb ecadf bdf
